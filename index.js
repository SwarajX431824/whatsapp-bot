...
client.on('qr', (qr) => {
    ...
    // Fallback URL to view QR cleanly in a web browser
...
// Primary listener: Captures incoming AND outgoing self-messages
client.on('message_create', async (msg) => {
    // 1. Ignore status updates completely
    if (msg.from === 'status@broadcast' || msg.to === 'status@broadcast') {
        return;
    }

    // 2. Ignore empty or non-text messages
    if (!msg.body || msg.body.trim() === '') {
        return;
    }

    // 3. Process commands starting with !reply
    if (msg.body.startsWith('!reply')) {
        ...
        try {
            const response = await ai.models.generateContent({
                ...
                contents: `Write ONLY the raw direct response message text to send back. Do NOT include preambles, bullet points, options, or extra text. Instruction: ${prompt}`,
            });

            console.log(`[SUCCESS] Gemini response generated: "${response.text}"`);
